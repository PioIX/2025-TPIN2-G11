...
useEffect(() => {
  ...
  const setupSocketListeners = () => {
    ...
    socket.on("iniciarJuego", (data) => {
      ...
      // Guardar información de la sala para usar en gameRoom
      localStorage.setItem('codigoSala', codigoSala);
      localStorage.setItem('esAnfitrion', esAnfitrion.toString());
      // Redirigir automáticamente a gameRoom después de un breve delay
      setTimeout(() => {
...
const iniciarJuego = () => {
  if (socket && esAnfitrion) {
    ...
    // Guardar información para gameRoom
    localStorage.setItem('codigoSala', codigoSala);
    localStorage.setItem('esAnfitrion', 'true');
    // El anfitrión también debe redirigirse después de un breve delay
    setTimeout(() => {
      router.push("/gameRoom");
    }, 2000);
  }
